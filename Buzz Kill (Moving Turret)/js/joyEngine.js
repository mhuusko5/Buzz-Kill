...
var characterSizer = 0.8;

Joy.Behaviour.define('bee', {
    desiredHeight: 0.084 * engine.height * characterSizer,
    desiredWidth: 0.06 * engine.height * characterSizer,
    ...
    UPDATE: function () {
        if (this.player.playing && this.alive) {
            var adjustedEngineUpdateTime = (this.player.usedBomb ? 2 : 1) * engineUpdateTime;
            ...
            for (var i = 0; i < this.player.onscreenShots.length; i++) {
                ...
                if (this.timeAlive > 2000 && shot && pointDistance(this.position, shot.position) < engineCollisionOffset && !shot.hurtsPlayer) {
            ...
            if (this.player.usingForcefield) {
                var distanceWithinForcefield = this.player.forceFieldRadius - pointDistance(this.position, this.player.position);
...
Joy.Behaviour.define('shot', {
    _width: (engineDiagonal / 35) * characterSizer,
    _height: (engineDiagonal / 31) * characterSizer,
    vector: null,
    timeSinceBounce: 0,
    bounceNum: 1,
    timers: null,
...
Joy.Behaviour.define('turret', {
    _width: engine.height * 0.264 * characterSizer,
    _height: engine.height * 0.264 * characterSizer,
    ...
    UPDATE: function () {
        if (this.player.playing) {
            ...
            var toTravel = Math.sqrt(Math.pow(this.desiredPosition.x - this.position.x, 2) + Math.pow(this.desiredPosition.y - this.position.y, 2));
            var slower = toTravel > 50 ? toTravel / 50 : 1;

            this.position.x += (this.desiredPosition.x - this.position.x) / (10 * slower);
            this.position.y += (this.desiredPosition.y - this.position.y) / (10 * slower);

            this.player.position.x = this.position.x;
            this.player.position.y = this.position.y;

            var pointing = {
                x: this.desiredPosition.x - this.position.x,
                y: this.desiredPosition.y - this.position.y
            };

            var newRotation = Math.atan2(pointing.y, pointing.x) * (180.0 / Math.PI) + 90.0;

            this.rotation += ((newRotation - this.rotation) / 8) % 360;
...
Joy.Behaviour.define('powerup', {
    desiredHeight: 0.082 * engine.height * characterSizer,
    desiredWidth: 0.082 * engine.height * characterSizer,
    timeTillLeave: 0,
    timeAlive: 0,
    timers: null,
    containingArray: null,
